...
import { db } from "@/lib/firebase"; // ✅ alias로 수정
...
//  const initialPage = parseInt(queryParams.get("page") || "0", 10);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      setInitialPage(page);
      setCurrentPage(page); // 👉 currentPage도 여기서 설정
    }
  }, []);
  ...
  const handleSelectSite = (siteData) => {
    ...
    setShowModal(false);
    router.push(`/preview?pages=${encodeURIComponent(JSON.stringify(fixedPages))}&headerType=${siteData.headerType || "헤더02"}`);
  };
...
const handleDeleteAllSites = async () => {
  ...
  const handleLoadSavedSite = async () => {
    ...
    if (fixedPages.length > 0) {
      router.push(`/preview?pages=${encodeURIComponent(JSON.stringify(fixedPages))}`);
    } else {
